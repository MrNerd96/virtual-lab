...
const HeartThread = ({ contraction }: { contraction: number }) => {
    ...
    const threadPath = useMemo(() => {
        ...
        // Start at heart (dynamic shortening), End at lever height
        const start = new THREE.Vector3(localX, 0.06 - (3.0 - contraction) * 0.03, localZ);
        const end = new THREE.Vector3(localX, leverY, localZ);

        return new THREE.LineCurve3(start, end);
    }, [contraction]);

    const startPoint = threadPath.v1;

    // Define Hook Shape (J-Curve)
    ...
    return (
        ...
        <group position={[0, 0.1, 0.5]}>
            {/* The Thread */}
            <Tube args={[threadPath, 64, 0.003, 8, false]}>
                <meshStandardMaterial color="#f0f0f0" roughness={0.5} />
            </Tube>

            {/* The Metal Hook (J-Shape) */}
            <group position={startPoint} rotation={[0, 0, 0.1]}>
                ...
                <div className="w-full lg:w-[450px] bg-slate-900 border-l border-slate-800 flex flex-col flex-1 lg:flex-none">
                    ...
                    <div className="p-4 lg:p-6 bg-slate-900 z-10 space-y-4 overflow-y-auto">
                        ...
                        <div className="text-[10px] text-slate-500 text-center leading-relaxed">
                            SA Node (Sinus Venosus) is the cardiac pacemaker. Temperature changes affect heart rate by altering the rate of impulse generation.
                        </div>
                    </div>
                </div>
            </main>
